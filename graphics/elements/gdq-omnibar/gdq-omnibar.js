(function () {
	...
	class GdqOmnibar extends Polymer.MutableData(Polymer.Element) {
		static get is() {
			return 'gdq-omnibar';
		}		
		ready() {
				...
				replicant.once('change', () => {
					...
					if (numDeclared >= replicants.length) {
						...
						leaderboard.on('change', newVal => {
						    ...
						    newVal.ranking.forEach(rank => {

						        if (rank.status != "Forfeit")
						            formattedRankings.push(rank);
						    });
		...
		currentRunChanged(newVal) {
			if (!newVal.name.toLowerCase().startsWith("the"))
				{
			this.name = "The " + newVal.name;
			} else 
		...
		run() {
			...
			function promisifyTimeline(tl) {
				return new Promise(resolve => {
					tl.call(resolve, null, null, '+=30.00'); //0.03
				});
			}
		...
		showCTA() {
			...
			if (formattedRankings.length > 0)
			{
			    ...
			    if (formattedRankings.length > 1)
			    {
			        for (var i = 1; i < formattedRankings.length; i++) {

			            offsetString = "-";
			            offsetString = offsetString.concat(offset.toString());
			            offsetString = offsetString.concat("%");
