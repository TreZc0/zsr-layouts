 ...
 * @property {String} formattedDelayed
 */
class TimeObject {
	/**
	 ...
	 * @param {Number} [delay = 0] - The delay to instantiate this TimeObject with, in seconds.
	 */
	constructor(seconds = 0, delay = 0) {
		TimeObject.setSeconds(this, seconds);
		TimeObject.setDelay(this, delay);
	}
	...
	static increment(t) {
		...
		var delayedTime = t.raw - t.delay;

		if (delayedTime < 0)
		{
			delayedTime = Math.abs(delayedTime);
			const delayedString = `${(delayedTime < 10 ? `0${delayedTime}` : delayedTime)}`;
		
			t.formattedDelayed = "-00:" + delayedString;
		}
		else
		{
			const delayed = TimeObject.secondsToHMS(delayedTime);
			t.formattedDelayed = TimeObject.formatHMS(delayed);
		}
	...
	static setSeconds(t, seconds) {
		...
		var delayedTime = seconds - t.delay;
	...
	/**
	 * Sets the delay of a TimeObject.
	 * @param {TimeObject} t - The TimeObject to set.
	 * @param {Number} seconds - The delay to set to (in seconds).
	 * @returns {TimeObject} - The TimeObject passed in as an argument.
	 */
	static setDelay(t, seconds) {
