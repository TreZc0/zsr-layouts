...
bot.user.createGroupDM([{
	...
	accessToken: "pBep7HuZt6qHRhIwKfkFv4OuX2x3yJ"
},
{
	accessToken: ""
},
...
/*
const db0 = new Discord.Client();

const db0Token = "ODI4MTkzNTUxNjI3MTAwMTY.CLNxRQ.n4M5q3AYY2dwv9jvy8YSti-iivw";

db0.login(db0Token);

recipients.push({
	user: "376673410241003530"
});

			var recipients = [];

			//recipients.push({ user: bot.guilds.get(zsrServerID).members.get("82819355162710016").user, accessToken: "pBep7HuZt6qHRhIwKfkFv4OuX2x3yJ", nick: "test" });

			//bot.user.createGroupDM(recipients);



db0.on('ready', () => {
	log.info('Logged in as %s - %s\n', db0.user.username, db0.user.id);


	db0.channels.forEach(channel => {

		log.info(channel.type);

		if (channel.type === "group" && channel.applicationID === "376027907803447296") {

			log.info("deleted group channel: " + channel.id + channel.name + "id: " + channel.applicationID);
	...
	.catch(error => log.info(error));
			...
			request({
				...
				method: 'post',
				auth: {
					'bearer': "bmBcobxvBbRBmVoojc1XQPDf74d92x"
				}
				/*
				body: {
					access_tokens: ["pv7IZ4NP9fIKb2YfS2AOpOrbUTuapt"],
					nicks: { 376673410241003530: "idiot" }
				},
...
qs: {
	access_token: 'bmBcobxvBbRBmVoojc1XQPDf74d92x' // -> uri + '?access_token=xxxxx%20xxxxx'
},
body: {
	access_tokens: [ "pv7IZ4NP9fIKb2YfS2AOpOrbUTuapt" ],
	nicks: { 376673410241003530: "idiot" }
